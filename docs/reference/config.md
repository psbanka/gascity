...
| `idle_timeout` | string |  |  | IdleTimeout is the maximum time an agent session can be inactive before the controller kills and restarts it. Duration string (e.g., "15m", "1h"). Empty disables idle checking — but an agent that declares no lifecycle key at all is composed with DefaultPoolIdleTimeout instead, so warm-idle sessions cannot linger indefinitely and strand routed work (ApplyPoolLifecycleDefaults, ga-r9s). |
